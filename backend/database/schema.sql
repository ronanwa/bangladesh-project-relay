...
* Scope: user, phone_call_record, report, optional_attachment (device, installation, sensor_reading deferred post-MVP)
* 
...
-- ============================================================
CREATE TABLE "user" (
    user_id         SERIAL PRIMARY KEY,         -- randomly generated user ID number (primary key)
...
CREATE TABLE phone_call_record (
    ...
    user_id         INT REFERENCES "user"(user_id),     -- associated user_id (foreign key)
...
CREATE TABLE report (
    ...
    user_id                  INT REFERENCES "user"(user_id),                                        -- nullable: anonymous report (foreign key)
...
-- ============================================================
-- OPTIONAL_ATTACHMENT TABLE
-- 
-- Photo / video / audio (voice note) a user deliberately attaches as evidence.
-- Distinct from phone_call_record, which is the phone call capture itself.
-- ============================================================
CREATE TABLE optional_attachment (
    ...
    user_id                  INT REFERENCES "user"(user_id),                    -- associated user ID (foreign key)
...
CREATE INDEX idx_optional_attachment_report_id ON optional_attachment (report_id);
CREATE INDEX idx_optional_attachment_user_id ON optional_attachment (user_id);
CREATE INDEX idx_phone_call_record_user_id ON phone_call_record (user_id);
...
CREATE INDEX idx_optional_attachment_location ON optional_attachment USING GIST (location);